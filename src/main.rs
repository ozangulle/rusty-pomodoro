...
use rusty_pomodoro::communication::{ConcSender, ConcReceiver};
use std::thread;

fn main() {
    let config = PomodoroConfig {
        pomodoro_time_in_secs: 1 * 60,
        short_break_time_in_secs: 1 * 60,
        long_break_time_in_secs: 1 * 60,
    };
    ...
    // pomodoro.add_update_observer(_cli.as_ref());
    // _pomodoro.start();
}
